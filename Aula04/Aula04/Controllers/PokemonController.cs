...
namespace Aula04.Controllers
{
    ...
    public class PokemonController : ControllerBase
    {
        ...
        public IActionResult BuscarPorCodigo(int codigo)
        {
            ...
            var resultado =
                from poke in lista
                where poke.id == codigo
                select poke;
            */

            var resultado = lista.Where(x => x.id == codigo);
        ...
        {
            //var resultado = lista.Where(x => x.name.Contains(nome, StringComparison.OrdinalIgnoreCase));

            var resultado =
                from poke in lista
                where poke.name.Contains(nome, StringComparison.OrdinalIgnoreCase)
                select poke;

            resultado = resultado.OrderBy(x => x.name);
